...
const AddExpense = (props) => {
  ...
  const getId = (id, isEdit) => {
    ...
    fetch(
      `https://expense-tracker-authentication-default-rtdb.firebaseio.com/expenseList/${username}/${id}.json`,
      {
        method: "DELETE",
      }
    )
  ...
  useEffect(() => {
    fetch(
      `https://expense-tracker-authentication-default-rtdb.firebaseio.com/expenseList/${username}.json`
    )
      .then((res) => {
        if (!res.ok) {
